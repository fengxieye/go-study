...
func TestClient_dialTimeout(t *testing.T) {
	...
	t.Run("timeout", func(t *testing.T) {
		...
		_assert(err != nil && strings.Contains(err.Error(), "dialTimeout"), "expect a timeout error")
	})

	t.Run("0", func(t *testing.T) {
		_, err := dialTimeout(f, "tcp", l.Addr().String(), &service.Option{ConnectTimeout: 0})
		_assert(err == nil, "0 is no timeout but err")
	})
}
...
func TestClient_Call(t *testing.T) {
	...
	t.Run("clinet timeout", func(t *testing.T) {
	...
	t.Run("server handle timeout", func(t *testing.T) {
		...
		_assert(err != nil && strings.Contains(err.Error(), "requset timeout"), "expect a timeout error")
	})
}
