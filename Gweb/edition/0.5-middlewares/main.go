...
func LoggerV2() gweb.HandleFunc {
	return func(ctx *gweb.Context) {
		t := time.Now()
		ctx.Fail(500, "Internal Server Error")
		log.Printf("[%d] %s in %v for group v2", ctx.StatusCode, ctx.Req.RequestURI, time.Since(t).Microseconds())
	}
}

func LoggerV1() gweb.HandleFunc {
	return func(ctx *gweb.Context) {
		t := time.Now()
		ctx.Next()
		log.Printf("[%d] %s in %v for group v1", ctx.StatusCode, ctx.Req.RequestURI, time.Since(t).Nanoseconds())
	}
}
